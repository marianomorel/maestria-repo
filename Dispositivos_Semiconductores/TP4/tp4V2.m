...
% Preallocate output matrix for ID
ID = zeros(length(VD), length(VGS)); 

% Calculate ID by segments for each VD value
for i = 1:length(VGS)
    for j = 1:length(VD)
        if VGS(i) > VT(1) && VD(j) < 0.75
            ID(j, i) = Kn(1) * (VGS(i) - VT(1) - m(1)/2 * VD(j)) * VD(j);
        elseif VGS(i) > VT(1) && VD(j) >= 0.75
            ID(j, i) = Kn(1) * ((VGS(i) - VT(1))^2) / (2 * m(1));
        elseif VGS(i) <= VT(1)
            ID(j, i) = Kn(1) * (m(1) - 1) * VTH^2 * exp((VGS(i) - VT(1)) / (m(1) * VTH)) * (1 - exp(-VD(j) / VTH));
        end
    end
end

% Plot ID vs VD curves for each VGS value
figure;
hold on;
for i = 1:length(VGS)
    plot(VD, ID(:, i), 'DisplayName', ['VGS = ' num2str(VGS(i)) 'V']);
end

% Add given current curves
plot(VD, ID_VGS_1_08, '-d', 'DisplayName', 'VGS = 1.08V');
plot(VD, ID_VGS_1_44, '-^', 'DisplayName', 'VGS = 1.44V');
plot(VD, ID_VGS_1_8, '-v', 'DisplayName', 'VGS = 1.8V');

% Configure plot
xlabel('VD (V)');
ylabel('ID (A)');
title('Curvas de ID vs VD para diferentes valores de VGS');
legend show;
grid on;
hold off;
                     ...
                     %#######CANAL CORTO########%
                                 ...
                                 %EJE1%
% Vectores dados
VG_CC = [0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1, 1.05, 1.1, 1.15, 1.2, 1.25, 1.3, 1.35, 1.4, 1.45, 1.5, 1.55, 1.6, 1.65, 1.7, 1.75, 1.8];
ID_CC = [-8.506e-10, -2.0816e-09, -1.5101e-09, 1.5473e-09, 1.136e-09, 3.1329e-09, 1.3669e-08, 6.9637e-08, 3.5038e-07, 1.0125e-06, 4.2691e-06, 1.4106e-05, 4.3253e-05, 0.00011091, 0.00023993, 0.00044668, 0.00073473, 0.001099, 0.0015372, 0.0020152, 0.0025243, 0.0030657, 0.0036406, 0.0042338, 0.00483, 0.0054334, 0.0060433, 0.0066517, 0.0072597, 0.0078697, 0.0084851, 0.0090903, 0.0096947, 0.0102961, 0.010899, 0.011505, 0.012096];

% Crear el plot
figure;
plot(VG_CC, ID_CC, 'b-o', 'LineWidth', 2);
grid on;
title('ID vs VG');
xlabel('VG (V)');
ylabel('ID (A)');
...
% Crear el plot semilogarítmico
figure;
semilogy(VG_CC, abs(ID_CC), 'b-o', 'LineWidth', 2); % Se utiliza abs(ID) para evitar problemas con valores negativos
grid on;
title('ID vs VG (Semilogarítmica)');
xlabel('VG (V)');
ylabel('ID (A) (Escala Logarítmica)');



                                    %EJE3%
ID_VG0_CC = [9.032e-10, -7.358e-10, 2.1404e-9, 6.271e-10, -1.09e-9, 8.087e-10, -2.017e-9, -1.039e-9, 1.4254e-9, -1.1357e-9, 8.186e-10, -2.0888e-9, -1.4989e-9, 2.1556e-9, 6.583e-10, -1.1233e-9, 2.0198e-9, 2.0998e-9, 2.1748e-9, 6.84e-10, -1.1462e-9, 2.0438e-9, 2.1202e-9, 5.706e-10, -1.0389e-9, 7.615e-10, -2.0507e-9, -9.93e-10, 2.2253e-9, 1.5879e-9, -2.0045e-9, -1.0601e-9, 7.843e-10, -2.041e-9, -1.0096e-9, 2.2338e-9, 8.552e-10];
ID_VG036_CC = [1.17e-11, 1.0729e-08, 1.8707e-08, 2.0158e-08, 2.1182e-08, 2.1921e-08, 2.519e-08, 2.6307e-08, 2.7377e-08, 2.8714e-08, 3.0064e-08, 3.2762e-08, 3.5404e-08, 3.7246e-08, 3.8793e-08, 4.0474e-08, 4.5066e-08, 4.686e-08, 4.8732e-08, 5.0299e-08, 5.2756e-08, 5.9578e-08, 5.9279e-08, 6.3889e-08, 6.6364e-08, 7.3177e-08, 7.2759e-08, 8.3454e-08, 8.0593e-08, 8.9276e-08, 9.4353e-08, 1.03229e-07, 6.3729e-08, 7.4908e-08, 1.02064e-07, 7.5127e-08, 8.3465e-08];
ID_VG072_CC = [-8.2812e-007, 5.837e-005, 8.4706e-005, 9.7347e-005, 0.000105972, 0.00011566, 0.00012062, 0.0001272, 0.00013238, 0.00013683, 0.00014281, 0.0001491, 0.00015444, 0.00015991, 0.00016637, 0.0001706, 0.0001761, 0.00018308, 0.00018911, 0.00019601, 0.00020194, 0.00020829, 0.00021546, 0.00022065, 0.00022632, 0.00023812, 0.00024541, 0.00025191, 0.00025816, 0.00026624, 0.00026138, 0.00027674, 0.0002868, 0.00029723, 0.00029684, 0.00030471, 0.0003155];
ID_VG108_CC = [1.4466e-006, 0.00068908, 0.0012345, 0.0016388, 0.0019242, 0.002121, 0.0022609, 0.0023621, 0.0024367, 0.0024976, 0.0025458, 0.0025926, 0.0026382, 0.0026796, 0.0027188, 0.0027512, 0.0027869, 0.0028193, 0.0028559, 0.0028904, 0.0029238, 0.0029499, 0.0029797, 0.0030169, 0.0030636, 0.0030946, 0.0031242, 0.0031421, 0.0031688, 0.0032168, 0.0032252, 0.0032601, 0.0033054, 0.0033413, 0.0033443, 0.0033822, 0.0034352];
ID_VG144_CC = [2.5089e-006, 0.0011909, 0.0022758, 0.0032253, 0.004022, 0.0046695, 0.005175, 0.0055674, 0.0058646, 0.0060951, 0.0062699, 0.0064083, 0.0065183, 0.0066113, 0.0066924, 0.0067681, 0.0068339, 0.0068887, 0.0069481, 0.0069995, 0.0070549, 0.0071057, 0.0071564, 0.0072131, 0.0072615, 0.0073069, 0.0073497, 0.0073931, 0.0074122, 0.0074625, 0.0075133, 0.0075532, 0.0075838, 0.0076373, 0.0076788, 0.0076974, 0.0077488];
ID_VG18_CC = [3.1361e-006, 0.0014699, 0.0028682, 0.0041597, 0.0053273, 0.0063583, 0.0072435, 0.007997, 0.0086256, 0.0091404, 0.0095584, 0.0098949, 0.0101628, 0.0103806, 0.0105583, 0.01071, 0.0108387, 0.0109492, 0.0110396, 0.0111239, 0.0112022, 0.011289, 0.011367, 0.011427, 0.011487, 0.011549, 0.011604, 0.011658, 0.011706, 0.011746, 0.011798, 0.011864, 0.011911, 0.011932, 0.011989, 0.012044, 0.01209];

% Definir el vector de voltajes VG para el eje X
VG = linspace(0, 1.8, length(ID_VG108_CC));

% Plotear las curvas
figure;
plot(VG, ID_VG0_CC, '-o', 'DisplayName', 'VG = 0V');
hold on;
plot(VG, ID_VG036_CC, '-s', 'DisplayName', 'VG = 0.36V');
plot(VG, ID_VG072_CC, '-d', 'DisplayName', 'VG = 0.72V');
plot(VG, ID_VG108_CC, '-^', 'DisplayName', 'VG = 1.08V');
plot(VG, ID_VG144_CC, '-p', 'DisplayName', 'VG = 1.44V');
plot(VG, ID_VG18_CC, '-h', 'DisplayName', 'VG = 1.8V');
