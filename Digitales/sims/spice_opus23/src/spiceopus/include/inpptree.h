...
// 1.	Macro PT_SPECSIG added for special signal node that can occur in parse tree.
//		For list of special signals see specSigList (bin/config.c file) or global
//		pointer ft_sim->specSigs, which points to the list. For special signal node
//		element valueIndex in structure INPparseNode is set to its index
//		specSigList.
//		Also an argument for array of special signal values added to declaration of
//		IFeval() function.
// Author: Janez Puhan
//////////
 ...
 *  tree->p.IFeval(&tree, result, vals, specsigs, derivs)
...
#define PT_VAR      8
#define PT_COMMA    10
#define PT_SPECSIG	11	// For special signal node.
...
//////////
// Modified code:
extern int IFeval(IFparseTree *, double, double *, double *, double *, double *);
extern int IFptdel(IFparseTree *);
extern void INPdeleteParseNode(INPparseNode *);
#else /* stdc */
extern int IFeval();
extern int IFptdel();
extern void INPdeleteParseNode();
#endif /* stdc */
