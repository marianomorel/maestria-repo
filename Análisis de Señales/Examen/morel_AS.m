...
ceros = [-1 -1]; % 
r = 0.92;
fc = 15; % Frecuencia de corte en 20Hz
titapb = fc*2*pi/Fs; % Todo el círculo es Fs = 1000Hz
...
muestreo_ventana = round(ventana * Fs);
...
%%
               %######EJERCICIO 5########
       %FSB=600hz
salida=out_notch(locs(1)+60:locs(1)+60+150);
figure;
plot(salida)
grid on
...
salida_T=salida./length(locs); %no me deja promediar arroja el siguiente error Index exceeds matrix dimensions.
